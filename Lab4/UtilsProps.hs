...
-- Generates a position within the range of the minefield and checks if
-- the adjacentPos method only returns valid positions at that position
prop_adjacentPos :: Minefield -> Property
prop_adjacentPos (Minefield m) = forAll (genPos (h,w)) (prop_adjacentPos' (h,w))
        where h = length m
              w = length (head m)

-- Checks if all positions generated by neighbourPos are valid
prop_adjacentPos' :: Pos -> Pos -> Bool
prop_adjacentPos' restrictions p = all (validPos restrictions) 
                                    (adjacentPos restrictions p)